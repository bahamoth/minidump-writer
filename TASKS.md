...
| T-022 | Refactor iOS handler_thread from Option<thread_t> to thread_t | TODO | - | - | iOS implementation unnecessarily uses Option<thread_t> for handler_thread while macOS correctly uses thread_t with mach_thread_self() fallback. This prevents reusing ActiveThreads helper and causes repeated thread filtering code. |
| T-023 | Implement ActiveThreads helper for iOS | TODO | - | T-022 | After fixing handler_thread design, implement ActiveThreads iterator similar to macOS to eliminate duplicate thread filtering logic across multiple streams. |
