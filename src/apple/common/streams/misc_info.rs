...
#[repr(C, packed(4))]
pub struct TaskThreadsTimeInfo {
...
///
/// We write a [`MINIDUMP_MISC_INFO_2`] to this stream, which includes
...
pub fn write_misc_info<T: TaskDumperHelper>(
    ...
    let pid = dumper
        .pid_for_task()
        .map_err(|_| MemoryWriterError::IOError)?;
    ...
    // CPU frequency information - only available on macOS
    #[cfg(target_os = "macos")]
    {
