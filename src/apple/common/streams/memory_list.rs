...
pub(crate) trait MemoryListStream {
    ...
    fn write_memory_list(
        ...
        if let Some(cc) = self.crash_context() {
            if cc.exception.is_some() {
                ...
                if let Some(ip_range) = get_ip_block(cc.thread) {
                    ...
                    self.memory_blocks_mut().push(MDMemoryDescriptor {
        ...
        let list_header =
            MemoryWriter::<u32>::alloc_with_val(buffer, self.memory_blocks_mut().len() as u32)?;
        ...
        let block_list = MemoryArrayWriter::<MDMemoryDescriptor>::alloc_from_array(
            buffer,
            self.memory_blocks_mut(),
        )?;
